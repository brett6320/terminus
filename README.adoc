...
image:https://dl.circleci.com/status-badge/img/gh/usetrmnl/terminus/tree/main.svg?style=svg[CircleCI Build Status,link=https://dl.circleci.com/status-badge/redirect/gh/usetrmnl/terminus/tree/main]
image:https://github.com/usetrmnl/terminus/actions/workflows/docker.yml/badge.svg[Docker Build Status,link=https://github.com/usetrmnl/terminus/actions/workflows/docker.yml]
...
When launching this application for the first time, there will be no users. The first user to register will automatically be given _Verified_ status and the _Admin_ role along with full access to the system. All subsequent users will be marked as _Unverified_ (and must be manually _Verified_ to gain access) with the _Member_ role.

Roles determine what a user may access:

* *Admin*: Full access, including user management (`/users`), the Sidekiq Web UI (`/sidekiq`), firmware and model management, and all delete/destructive actions.
* *Member*: Access to day-to-day content -- devices, playlists, screens, and extensions -- but not the admin-only areas above. Requests to admin-only paths are answered with a `403 Forbidden`.
...
* Visit `/multifactor-manage` to enroll in multifactor authentication using a TOTP authenticator app or a passkey (WebAuthn), and to manage recovery codes. Set `MFA_REQUIRED=true` (see {doc_configuration_link}) to force all users to enroll before using the web UI.

=== Auditing

Every successful, authenticated, state-changing request (create, update, delete), along with authentication events (login, logout, account creation, and password/email changes), is recorded to a tamper-evident audit trail. Each event is hash-chained to the previous one (`digest = SHA256(actor, action, entity, metadata, previous digest)`), so any modification, deletion, or reordering of past events can be detected by re-walking the chain via `Terminus::Aspects::Audit::Verifier`. This provides tamper-_evidence_, not tamper-_prevention_: a party with direct database access could recompute the entire chain, so for stronger guarantees pair it with append-only database grants, off-host log shipping, or periodic external anchoring of the latest digest.
